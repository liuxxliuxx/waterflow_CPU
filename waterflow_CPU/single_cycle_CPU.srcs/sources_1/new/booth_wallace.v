module booth_wallace(
    ...
    wire [63:0] s1_s [0:4]; wire [63:0] s1_c [0:4];
    ...
    wire [63:0] s2_s [0:3]; wire [63:0] s2_c [0:3];
    ...
    wire [63:0] s3_s [0:1]; wire [63:0] s3_c [0:1];
    csa_64bit st3_0(.in1(s2_s[0]), .in2(s2_c[0]), .in3(s2_s[1]), .sum(s3_s[0]), .carry(s3_c[0]));
    csa_64bit st3_1(.in1(s2_c[1]), .in2(s2_s[2]), .in3(s2_c[2]), .sum(s3_s[1]), .carry(s3_c[1]));

    wire [63:0] s4_s [0:1]; wire [63:0] s4_c [0:1];
    csa_64bit st4_0(.in1(s3_s[0]), .in2(s3_c[0]), .in3(s3_s[1]), .sum(s4_s[0]), .carry(s4_c[0]));
    csa_64bit st4_1(.in1(s3_c[1]), .in2(s2_s[3]), .in3(s2_c[3]), .sum(s4_s[1]), .carry(s4_c[1]));

    wire [63:0] s5_s; wire [63:0] s5_c;
    csa_64bit st5_0(.in1(s4_s[0]), .in2(s4_c[0]), .in3(s4_s[1]), .sum(s5_s), .carry(s5_c));

    wire [63:0] final_sum; wire [63:0] final_carry;
    csa_64bit st6_0(.in1(s5_s), .in2(s5_c), .in3(s4_c[1]), .sum(final_sum), .carry(final_carry));
    ...
    assign Product = {adder_high_out, adder_low_out};

...
// One-transaction asynchronous bridge between the 25 MHz SoC clock and the
// MIG UI clock. Source payload is held stable from its request toggle until
// the matching response returns, making it safe to sample in the UI domain.
module ddr_cdc_bridge (
    ...
    input  wire [31:0] src_req_wdata,
    output wire        src_resp_valid,
    input  wire        src_resp_ready,
    output wire [31:0] src_resp_rdata,
    ...
    output wire [31:0] dst_req_wdata,
    input  wire        dst_resp_valid,
    input  wire [31:0] dst_resp_rdata
);

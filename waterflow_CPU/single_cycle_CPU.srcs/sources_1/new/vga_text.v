module vga_text(
    ...
    // The text store needs an asynchronous CPU read for the existing MMIO
    // protocol.  The pixel-side register below supplies the display pipeline.
    (* ram_style = "distributed" *) reg [19:0] char_mem [0:2399];

    // One CPU-clock write port and one pixel-clock synchronous read port form
    // a simple dual-port, dual-clock framebuffer RAM.
    ...
    // Do not gate the BRAM enable directly with rst: rst is generated by an
    // asynchronously-reset synchronizer in soc_top, which would turn into an
    // asynchronous RAMB36 control path.  The clear state itself is reset
    // synchronously and rewrites the whole framebuffer before it is exposed.
    ...
    // Text writes and text clearing share the CPU-clock write port.
    ...
    // Framebuffer storage has exactly one writer in the CPU clock domain.
    always @(posedge cpu_clk) begin
        if (frame_write_enable)
            frame_mem[frame_write_addr] <= frame_write_data;
    end

    // Image registers, stream cursor, and framebuffer clear state.
    always @(posedge cpu_clk) begin
        if (rst) begin
            image_clear_busy <= 1'b1;
            image_clear_addr <= 19'd0;
    ...
    // All display metadata is delayed with the synchronous framebuffer read.
    always @(posedge pix_clk) begin
